...
- 有人下注后进入实时盘：押赢/押输的点数会轻微修正下一笔下注的报价，但近期表现始终占主导。默认资金池最多影响 20% 的胜率估计，再大的单边下注也不能盖过战绩。
- 每笔下注按提交前显示的赔率立即锁定，之后其他人的下注和资金池变化不会修改这笔赔率；猜中按锁定赔率返还，猜错损失本金。默认最低赔率为 1.30，90% 返还率和 5000 点基础流动性用于维持可玩性并限制对刷。
...
- `PREDICTION_MARKET_MAX_POOL_INFLUENCE`：资金池对胜率估计的最大影响，默认 0.20。
