...
PREDICTION_MARKET_PAYOUT_RATE = min(
    1.0, max(0.5, float(os.getenv("PREDICTION_MARKET_PAYOUT_RATE", "0.90")))
)
PREDICTION_MARKET_MAX_POOL_INFLUENCE = min(
    0.49, max(0.0, float(os.getenv("PREDICTION_MARKET_MAX_POOL_INFLUENCE", "0.20")))
)
...
PREDICTION_DEATH_SECONDS = max(
    60, int(os.getenv("PREDICTION_DEATH_SECONDS", "259200"))
)
